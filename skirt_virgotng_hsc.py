...
def get_subhalos(basePath,snap,mstar_lower=0,mstar_upper=np.inf):
    ...
    fields = ['SubhaloMassType','SubhaloGrNr','SubhaloFlag']
    
    if not 'IllustrisTNG' in basePath:
        fields.remove('SubhaloFlag')
        subhalos = il.groupcat.loadSubhalos(basePath,snap,
                                            fields=fields)
        if not subhalos['count']==0:
    ...
    subs = np.arange(subhalos['count'],dtype=int)
...
def prepare_skirt(universe,simulation,snap,sub,sim_path,tmp_path,
    ...
    if not os.access(f'{tmp_path}/{f_gas}',0):
        ...
        #create 2D-array and write into data file
        if universe=='IllustrisTNG': 
            total_gas = np.column_stack((coords,rho_dust))
        else:
            hsml = it.convertlength(gas['SubfindHsml'][gas_idx])*a2*1000
            m_dust = it.convertmass(gas['Masses'][gas_idx])*rho_dust/rhog
            total_gas = np.column_stack((coords,hsml,m_dust))
...
def run_skirt(universe, simulation,
              snap,sub,cam,sim_path,tmp_path,skirt_path,out_path,
    ...
    fovsize,npix,ncells,nsources,fof_factor,f_stars,f_mappings,f_gas = prepare_skirt(universe,simulation, snap,sub,sim_path,tmp_path,correct_pdrs=False)
    ...
    ski_in = f'{skirt_path}/template_{universe}.ski'
    #ski_in = f'{skirt_path}/template_IllustrisTNG.ski'
    ...
    for i in range(len(bands)):
        ...
        hdr['SIMTAG'] = (simulation,'Simulation name')
...
def prepare_only(args):
    ...
    project_path = f'/vera/ptmp/gc/bconn/SKIRT/{universe}'
    
    for snapnum in snaps:
        # photometry path
        phot_path = f'{project_path}/{simulation}/HSCSSP/Idealized/{snapnum:03}'
        subs,mstar = get_subhalos(sim_path,snap=snapnum,
                                  mstar_lower=mstar_lower)
        subs = subs[task_idx::ntasks]
        
        for subfindid in subs:
            # Check if output already exists. If so, skip.
            outfiles = glob.glob(
                f'{phot_path}/shalo_{snapnum:03}-{subfindid}_*_photo.fits')
            if len(outfiles)==len(cams):
                print(f'All output images already exist for {simulation}-{snapnum:03}-{subfindid}.')
                continue
           
            # working directory for job
            tmp_path=f'/vera/ptmp/gc/bconn/SKIRT/{universe}/{simulation}/tmpdir/tmp_{snapnum:03}-{subfindid}'
            if not os.access(tmp_path,0):
                os.system(f'mkdir -p {tmp_path}')
            os.chdir(tmp_path)
            
            fovsize,npix,ncells,nsources,fof_factor,f_stars,f_mappings,f_gas = prepare_skirt(universe,simulation,snapnum,subfindid,sim_path,tmp_path,correct_pdrs=False)
...
def run_only(args):
    ...
    for snapnum in snaps:
        ...
        for sub in subs:
            # working directory for job
            tmp_path=f'/vera/ptmp/gc/bconn/SKIRT/{universe}/{simulation}/tmpdir/tmp_{snapnum:03}-{sub}'
            skirt_path = f'/u/bconn/Projects/Simulations/IllustrisTNG/Scripts/SKIRT'

            f_stars = f'{tmp_path}/shalo_{snapnum:03d}-{sub}_stars.dat'
            f_mappings = f'{tmp_path}/shalo_{snapnum:03d}-{sub}_mappings.dat'
            f_gas = f'{tmp_path}/shalo_{snapnum:03d}-{sub}_gas.dat'

            inputs_ready = True
            for skirt_file in [f_stars,f_mappings,f_gas]:
                if not os.access(skirt_file,0):
                    print(f'Missing SKIRT input files for {snapnum:03}-{sub}. Skipping...')
                    inputs_ready = False
            ...
            for cam in cams:
                if not os.access(f'{phot_path}/shalo_{snapnum:03}-{sub}_{cam}_photo.fits',0):
                    run_skirt(universe,simulation,snapnum,sub,cam,
                             sim_path=sim_path,
                             tmp_path=tmp_path,
